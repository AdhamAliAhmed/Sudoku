...
//helper method that converts a string to a char array
...
void navigateToUrl(string url)
{
    try
    {
        string shellCommand = "google-chrome " + url;

        system(toCharArray(shellCommand));
    }
    catch (exception ex)
    {
        cout << "Google Chrome is required for this operation";
        cout.flush();
    }
}
...
void loadTemplate(int host[9][9], string directory)
{
    ...
    if (stream.is_open())
    {
        ...
        // int pointerPos = (81 * templateIndex);
...
struct mainGameScene
{
    ...
    void loadResources(string difficulty)
    {
        ...
        googleRegular.loadFromFile(fontsPath + "ProductSans-Regular.ttf");
        googleBold.loadFromFile(fontsPath + "ProductSans-Bold.ttf");
        googleBlack.loadFromFile(fontsPath + "ProductSans-Black.ttf");
    }
